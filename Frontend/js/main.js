...
async function saveGameResult(username, opponent, winner, result) {
  try {
...
function checkStatus(color) {
  ...
  // Checkmate
if (game.in_checkmate()) {
  let loser = game.turn(); // 'w' or 'b'
  let winner = loser === 'w' ? 'black' : 'white';

  let winnerPlayer = winner === 'white' ? 'nitish' : 'computer';
  saveGameResult("nitish", "computer", winnerPlayer, "checkmate");

  $('#status').html(`<b>Checkmate!</b> ${winner} wins!`);
  return true;
}
  ...
  // Draw (insufficient material)
    ...
    saveGameResult("nitish", "computer", "none", "draw");
